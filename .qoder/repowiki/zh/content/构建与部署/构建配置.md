...
## 更新摘要
**变更内容**
- 新增 PowerShell Web 构建脚本集成，提供完整的 Web/PWA 构建自动化支持
- 增强环境检查功能，包括 Node.js 版本验证和 npm 可用性检查
- 新增 AJV 版本冲突自动修复机制，解决 Angular 19 兼容性问题
- 完善构建模式支持，包括 build、dev、check 三种模式
- 增强错误处理和用户反馈机制
...
- [ngsw-config.json:1-31](file://ngsw-config.json#L1-L31)
- [capacitor.config.ts:6](file://capacitor.config.ts#L6)
- [scripts/windows/build_web_bywin.ps1:200-255](file://scripts/windows/build_web_bywin.ps1#L200-L255)
...
### 环境检查与验证
脚本包含完整的环境检查机制：
- **Node.js 版本检查**：要求 v18.19.1+ / v20.11.1+ / v22+
- **npm 可用性检查**：验证 npm 命令可用性
- **AJV 版本冲突自动修复**：自动检测并修复 ajv v6 到 v8 的升级问题

### 依赖管理与自动修复
- **自动依赖安装**：检测 node_modules 缺失时自动执行 npm install
- **AJV 版本冲突处理**：自动修复 Angular 19 兼容性问题
- **构建缓存清理**：支持 -Clean 参数清理 Angular 构建缓存
...
- **构建优化开关**
  - development 配置关闭构建优化与 SourceMap，便于调试；生产环境开启优化与哈希
- **Service Worker 缓存策略**
