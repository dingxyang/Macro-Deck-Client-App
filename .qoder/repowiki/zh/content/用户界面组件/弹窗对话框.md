...
## 结论
弹窗体系围绕“模态展示 + 数据回传 + 事件联动”的设计展开，既保证了用户体验的一致性，又通过清晰的生命周期与数据流实现了高内聚低耦合。AddConnection 与 SettingsModal 分别承担“连接配置”和“应用配置”的核心职责，其余弹窗作为支撑组件完善了错误提示与流程引导。建议在扩展新弹窗时复用现有模式：明确 props 与回传数据、规范 onWillDismiss 处理、及时清理订阅与资源。
