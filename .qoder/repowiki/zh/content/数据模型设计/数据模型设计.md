...
- 连接配置 Connection
  - 字段与类型
    ...
    - index: number | undefined（排序索引）
    - autoConnect: boolean | undefined（自动连接）
    - usbConnection: boolean | undefined（是否 USB 连接）
...
- 微件数据 Widget
  - 字段与类型
    ...
    - colSpan: number（跨列数）
    - rowSpan: number（跨行数）
    - backgroundColorHex: string | undefined（背景色，十六进制）
    - widgetContentType: WidgetContentType（内容类型枚举）
    - widgetContent: WidgetContent | undefined（具体内容）
  - 验证规则
...
- 按钮微件内容 ButtonWidget
  - 字段与类型
    - iconBase64: string | undefined（图标 Base64）
    - labelBase64: string | undefined（标签 Base64）
  - 验证规则
...
- 微件交互 WidgetInteraction
  ...
  - 验证规则
    - 枚举值必须属于预定义集合
  - 业务含义
    - 描述一次用户与微件的交互事件（如按下、释放、长按等）

- 协议按钮 Protocol2Button
  - 字段与类型
    - IconBase64: string | undefined（图标 Base64）
    - Position_X: number（网格列位置）
    - Position_Y: number（网格行位置）
    - LabelBase64: string | undefined（标签 Base64）
    - BackgroundColorHex: string | undefined（背景色）
  - 验证规则
...
- 快速设置 QR 数据 QuickSetupQrCodeData
  - 字段与类型
    ...
    - networkInterfaces: string[]（可用网络接口地址列表）
    - port: number（端口）
    - ssl: boolean（是否启用 SSL）
    - token: string（认证令牌）
  - 验证规则
...
- WidgetInteractionType
  - ButtonPress、ButtonShortPressRelease、ButtonLongPress、ButtonLongPressRelease
- ScreenOrientationType
  - Auto、Landscape、LandscapeAlt、Portrait、PortraitAlt
- AppearanceType
  - System、Dark、Light
- NavigationDestination
  - Home、Deck、ConnectionLost
