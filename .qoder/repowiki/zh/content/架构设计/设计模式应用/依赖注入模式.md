...
## 更新摘要
**所做更改**
- 更新了服务层架构分析，反映架构重构后的服务依赖关系
- 重新整理了核心服务依赖图，突出 WebSocket 通信链路
- 更新了依赖注入最佳实践，强调精简服务设计
- 新增了服务间解耦策略和循环依赖避免方法
- 完善了协议处理架构的依赖关系分析
...
## 简介
本文件系统性梳理 Macro-Deck-Client-App 中的依赖注入（DI）模式应用，围绕 Angular 依赖注入容器的工作原理展开，结合项目实际代码，解释@Injectable装饰器的使用、构造函数注入与可选依赖、单例模式与生命周期、服务间依赖关系管理、以及在组件与服务中的典型实践。经过架构重构后，项目采用了更加精简的服务设计，重点强化了 WebSocket 通信链路和协议处理架构，同时保持了清晰的依赖关系和良好的可维护性。

## 项目结构
项目采用 Angular + Ionic 架构，入口为 main.ts 引导 AppModule，AppModule 统一导入页面模块与第三方模块；业务服务均以@Injectable({ providedIn: 'root' })声明为根级单例，供全局任意组件按需注入。经过重构后，服务层更加精简，核心通信链路由 WebSocketService、ProtocolHandlerService 和 Protocol2Service 三者构成，形成了清晰的职责分离。
...
ws_svc --> loading_svc["loading.service.ts<br/>加载管理"]
proto_handler --> proto2_svc["protocol2.service.ts<br/>协议2处理"]
proto2_svc --> macro_deck_svc["macro-deck.service.ts<br/>面板数据管理"]
```
...
- [src/app/services/protocol/protocol2.service.ts:16-18](file://src/app/services/protocol/protocol2.service.ts#L16-L18)
...
- **重构后的服务层**
  - SettingsService、ConnectionService、WebsocketService、NavigationService、ProtocolHandlerService、Protocol2Service、MacroDeckService 等均以@Injectable({ providedIn: 'root' })声明，天然单例，跨组件共享状态与行为。
  - 服务层经过精简，去除了冗余组件，强化了核心通信链路。
- **组件层**
...
## 架构总览
下图展示应用启动、服务单例化与组件消费的关键流程，突出了重构后的核心通信链路：
...
WS->>ProtoHandler : 处理协议消息
ProtoHandler->>Proto2 : 分发协议2消息
Proto2->>MacroDeck : 更新面板数据
```

**图表来源**
- [src/main.ts:13-14](file://src/main.ts#L13-L14)
- [src/app/app.module.ts:19-42](file://src/app/app.module.ts#L19-L42)
- [src/app/services/websocket/websocket.service.ts:63-77](file://src/app/services/websocket/websocket.service.ts#L63-L77)
...
- **providedIn: 'root'**
  - SettingsService、ConnectionService、WebsocketService、NavigationService、ProtocolHandlerService、Protocol2Service、MacroDeckService 等均使用该选项，确保服务在应用根注入器中创建一次，随后通过 DI 容器全局复用。
...
### 构造函数注入与服务间依赖
- **WebsocketService 依赖链**
  - 依赖 LoadingService、ModalController、SettingsService、ProtocolHandlerService、NavigationService
  - 体现"高内聚、低耦合"：WebSocket 专注于通信，其他职责委托给对应服务。
  - 通过构造函数注入，组件与服务在编译期即明确依赖关系，利于测试与维护。
- **HomePage 依赖**
  - 依赖 SettingsService、ConnectionService、WebsocketService、WakelockService、PingService 等
  - 组件只关心自身视图逻辑，业务能力由注入的服务承担。
- **ProtocolHandlerService 依赖**
  - 依赖 Protocol2Service，通过协议版本分发消息，保持扩展性与可测试性。
...
WebsocketService --> ProtocolHandlerService : "注入"
ProtocolHandlerService --> Protocol2Service : "注入"
Protocol2Service --> MacroDeckService : "注入"
```
...
- [src/app/services/macro-deck/macro-deck.service.ts:16-30](file://src/app/services/macro-deck/macro-deck.service.ts#L16-L30)
...
- **重构后的协议处理链路**
  ...
  - ProtocolHandlerService 依赖 Protocol2Service，形成清晰的协议处理层次。
- **设置与连接的协同**
...
- **重构后的服务依赖图**
  - WebsocketService 是中枢服务，依赖 SettingsService、NavigationService、ProtocolHandlerService、LoadingService；ConnectionService 依赖 SettingsService；HomePage/DeckPage 依赖多个服务。
  - 协议处理链路：WebsocketService → ProtocolHandlerService → Protocol2Service → MacroDeckService。
- **环境差异**
...
Protocol2["Protocol2Service"] --> Websocket
Websocket --> MacroDeck["MacroDeckService"]
Loading["LoadingService"] --> Websocket
...
- **协议处理问题**
  - 如遇协议消息处理异常，检查 ProtocolHandlerService 的协议版本设置和 Protocol2Service 的消息处理逻辑。
...
## 结论
本项目在 Angular 依赖注入框架下，采用@Injectable({ providedIn: 'root' }) 的根级单例模式，结合构造函数注入与事件驱动机制，实现了清晰的服务边界与稳定的组件协作。经过架构重构后，服务层更加精简，核心通信链路由 WebSocketService、ProtocolHandlerService 和 Protocol2Service 三者构成，形成了高效的消息处理管道。通过环境变量与导航服务的配合，满足了 Web 与原生双端差异化需求。建议在后续扩展中继续保持单一职责与清晰依赖，避免循环依赖，必要时采用延迟注入与可选依赖策略，确保架构可演进与可维护。
