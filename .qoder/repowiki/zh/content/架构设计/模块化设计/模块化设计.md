...
## 简介
本文件面向 Macro-Deck-Client-App 的模块化设计，系统性阐述应用的模块化架构与组织方式，重点包括：
- 根模块 AppModule 的职责与配置
- 功能模块划分原则：页面模块（home、deck、web-home）、组件模块（widget-content-components）、共享模块等
- 模块间的依赖关系与导入/导出机制
- 延迟加载策略与性能优化考量
- 模块设计最佳实践与扩展指南

## 项目结构
应用采用基于功能域的模块化组织方式，结合 Ionic/Angular 的页面模块与组件模块划分，配合共享组件与服务，形成清晰的层次结构。
...
subgraph "共享与服务"
nav_service["src/app/services/navigation/navigation.service.ts<br/>导航服务"]
...
- [src/app/pages/shared/modals/settings-modal/settings-modal.component.ts:1-300](file://src/app/pages/shared/modals/settings-modal/settings-modal.component.ts#L1-L300)
...
- [src/app/pages/web-home/web-home.module.ts:10-20](file://src/app/pages/web-home/web-home.module.ts#L10-L20)
...
### 根模块 AppModule 设计
- 职责：集中导入页面模块、组件模块、Ionic、HTTP、存储、表单、Service Worker 等，统一引导应用。
- 关键点：
...
**章节来源**
- [src/app/app.component.ts:17-68](file://src/app/app.component.ts#L17-L68)
- [src/app/app.component.ts:38-39](file://src/app/app.component.ts#L38-L39)

## 依赖分析
- 模块导入关系
  - AppModule 导入所有页面模块与组件模块，并引入 Ionic、HTTP、存储、表单、Service Worker。
...
**章节来源**
- [src/app/app.module.ts:31-35](file://src/app/app.module.ts#L31-L35)
- [angular.json:44-46](file://angular.json#L44-L46)
- [angular.json:69](file://angular.json#L69)
- [angular.json:71-86](file://angular.json#L71-L86)
- [angular.json:87-107](file://angular.json#L87-L107)
...
## 结论
该应用通过明确的模块划分与清晰的依赖边界，实现了页面、组件与服务的解耦。根模块承担装配职责，页面模块聚焦业务视图，组件模块提升复用性，导航服务提供统一的页面切换能力。结合 Service Worker 与构建配置，整体具备良好的性能与可维护性。后续可在路由层面引入懒加载，进一步优化首屏加载与包体大小。
